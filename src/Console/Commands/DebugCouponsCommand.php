...
class DebugCouponsCommand extends Command
{
    protected $signature = 'coupons:debug';
    protected $description = 'Debug Coupons module loading';

    public function handle()
    {
        $this->info('🔍 Debugging Coupons Module...');
        
        // Check which route file is being loaded
        $this->info("\n📍 Route Loading Priority:");
        $moduleRoutes = base_path('Modules/Coupons/routes/web.php');
        $packageRoutes = base_path('packages/admin/coupons/src/routes/web.php');
        ...
        $viewPaths = [
            'Module views' => base_path('Modules/Coupons/resources/views'),
            'Published views' => resource_path('views/admin/coupon'),
            'Package views' => base_path('packages/admin/coupons/resources/views'),
        ];
        ...
        $controllerClass = 'Modules\\Coupons\\app\\Http\\Controllers\\Admin\\CouponManagerController';
        ...
        foreach ($routes as $route) {
            ...
            if (isset($action['controller']) && str_contains($action['controller'], 'CouponManagerController')) {
                $couponRoutes[] = [
                    'uri' => $route->uri(),
        ...
        if (!empty($couponRoutes)) {
            $this->table(['URI', 'Methods', 'Controller', 'Name'], $couponRoutes);
        } else {
            $this->warn("No coupon routes found.");
        }
    }
}
